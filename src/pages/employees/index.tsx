...
import { useRoqClient, useEmployeeFindManyWithCount } from 'lib/roq';
...
export function EmployeeListPage(props: EmployeeListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['first_name.contains', 'last_name.contains', 'position.contains'],
          ...params.filters,
        },
        'employee',
      ),
  ...
  const { data, error, isLoading, mutate } = useEmployeeFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.employee.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'hourly_rate', header: 'hourly_rate', accessorKey: 'hourly_rate' },
    { id: 'hours_per_week', header: 'hours_per_week', accessorKey: 'hours_per_week' },
